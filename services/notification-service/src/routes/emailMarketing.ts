/**
 * Email Marketing routes — ported from backend/src/routes/emailMarketing.ts.
 *
...
// Per-org rate limit on the send-campaign endpoint.
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      // Best-effort EmailLog row so the test-send is visible in audit/analytics.
      try {
        await prisma.emailLog.create({
          data: {
            organizationId,
            recipient: req.body.testRecipient,
            subject,
        ...
        console.error("[email-marketing] EmailLog write failed:", logErr);
      }

      await sendEmail({ to: req.body.testRecipient, subject, html });
