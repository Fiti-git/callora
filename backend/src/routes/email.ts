/**
 * Email compliance routes (Phase 2 Agent 9).
 *
 *   GET  /api/email/unsubscribe?t=<jwt>       browser one-click → redirect
 *   POST /api/email/unsubscribe?t=<jwt>       Gmail one-click → 204
 *
 * The JWT is signed with NEXTAUTH_SECRET and carries:
 *   { emailLogId, orgId, recipient }
 *
 * The endpoints are public (no auth) — the JWT IS the auth.
 */
...
interface UnsubPayload {
  ...
  orgId?: string | null;
  recipient: string;
  // Phase 3 Agent 10 — optional fields when the unsub link is sent from a
  // marketing campaign. Allow per-send + per-campaign accounting.
  campaignId?: string;
  sendId?: string;
}
...
async function applyUnsubscribe(payload: UnsubPayload): Promise<void> {
  ...
  // Phase 3 Agent 10 — keep marketing analytics in sync.
  if (payload.sendId) {
    try {
