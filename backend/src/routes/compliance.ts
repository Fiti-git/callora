/**
 * TCPA + CAN-SPAM compliance routes (Phase 2 Agent 9).
 *
...
router.get("/dnc", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const items = (hasMore ? rows.slice(0, limit) : rows).map((r) => ({
      id: r.id,
      // We don't have plaintext — UI will only show last4 if it stored locally;
      // for now expose the source/reason/timestamps. Hash is intentionally not returned.
      source: r.source,
      reason: r.reason,
