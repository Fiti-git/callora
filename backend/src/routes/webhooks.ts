/**
 * Tenant webhook management routes (Phase 3 Agent 12).
 *
 * Endpoints under /api/webhooks (tenant ADMIN only):
 *   POST   /webhooks                                — create (returns secret ONCE)
 *   GET    /webhooks                                — list (no secret)
 *   GET    /webhooks/:id                            — detail (no secret)
 *   POST   /webhooks/:id/regenerate-secret          — rotate (returns new secret ONCE)
 ...
 *   POST   /webhooks/:id/test                       — synthetic test.ping
 *
 * Zapier glue (no tenant JWT — auth via Organization.zapierTriggerToken):
 *   GET    /webhooks/zapier/trigger-url             — auth required, returns subscribe URL
 *   POST   /webhooks/zapier/subscribe?token=&url=&event=
 *   DELETE /webhooks/zapier/unsubscribe?token=&webhookId=
 *
 *   How to connect to Zapier:
 *     1. As an ADMIN call GET /api/webhooks/zapier/trigger-url. The response
 *        contains a long-lived URL Zapier should POST to subscribe.
 *     2. Zapier (or any 3rd-party) POSTs to that URL with `url` (its hook
 *        endpoint) and `event` (comma-separated WebhookEvent enum values).
 *        That creates a TenantWebhook on the matching org.
 *     3. To unsubscribe, Zapier hits DELETE with the same trigger token.
 */
...
router.get("/", async (req: Request, res: Response) => {
  ...
  const rows = await prisma.tenantWebhook.findMany({
    ...
    select: {
      ...
      createdAt: true,
      updatedAt: true,
      // intentionally NOT returning `secret`
    },
  });
  res.json(rows);
});
  ...
  async (req: Request, res: Response) => {
    ...
    const fresh = await prisma.webhookDelivery.create({
      data: {
        webhookId: wh.id,
        organizationId,
        // Use any subscribed event as the carrier; payload type marks it as test.
