...
/**
 * Dunning email templates. Subject + HTML are produced by `dunningEmail(key,
 * params)` so the worker can drive the cadence with a single call.
 *
 * The exported `subjectFor` helper is used by tests to assert the right
 * template was selected at each transition without needing to render HTML.
 */
