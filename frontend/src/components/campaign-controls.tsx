...
export function CampaignControls({
  ...
  async function handleFindLeads() {
    ...
    try {
      ...
      toast.error("Failed to find leads: " + (error instanceof Error ? error.message : "Unknown error"));
    } finally {
      setLoading(false);
    }
  }
  ...
  async function executeStartCalls() {
    ...
    try {
      ...
      toast.error("Failed to start calls: " + (error instanceof Error ? error.message : "Unknown error"));
      setIsConfirmOpen(false);
    } finally {
      setLoading(false);
    }
  ...
  async function handleRunFollowUps() {
    ...
    try {
      ...
      toast.error("Follow-ups failed: " + (error instanceof Error ? error.message : "Unknown error"));
    } finally {
      setLoading(false);
    }
  }
    ...
    <>
      ...
      <div className="flex items-center gap-2">
        ...
        {campaignType === "AI" && canAction && (
          <div className="flex items-center rounded-lg border border-gray-300 dark:border-gray-700 overflow-hidden bg-white dark:bg-gray-800">
            <span className="px-2.5 py-2 text-xs text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-700 border-r border-gray-300 dark:border-gray-700">
              Limit
            </span>
            <input
              type="number"
              ...
              className="w-14 px-2 py-2 text-sm text-gray-900 dark:text-white bg-white dark:bg-gray-800 focus:outline-none"
              min={1}
              max={100}
            />
          </div>
        ...
        {campaignType === "AI" && canAction && (
          <button
            ...
            disabled={loading}
            className="inline-flex items-center gap-1.5 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 disabled:opacity-50 transition-colors"
          >
